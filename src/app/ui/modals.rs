use crate::app::protocol::{AppAction, SetCrs};
...
impl OmapMaker {
            ...
            egui::Sides::new().show(
                ...
                |ui| {
                    ...
                    if ui.button("Yes, drop the files!").clicked() {
                        self.dispatch_action(AppAction::SetCrs(SetCrs::DropAll));
                    }
                },
            );
        });
            ...
            egui::Sides::new().show(
                ...
                |ui| {
                    if ui
                        ...
                        .clicked()
                    {
                        self.dispatch_action(AppAction::SetCrs(SetCrs::SetAllEpsg));
                    };
            ...
            egui::Sides::new().show(
                ...
                |ui| {
                    if ui.add_enabled(crs_set, egui::Button::new("Done")).clicked() {
                        self.dispatch_action(AppAction::SetCrs(SetCrs::SetEachCrs));
                    }
                    if ui.button("Go back").clicked() {
                        self.dispatch_action(AppAction::OpenModal(OmapModal::ManualSetCRS));
                    }
            ...
            ui.horizontal(|ui|{
                if self.gui_variables.lidar.unique_crs.is_empty() {
                    ...
                    if ui.button("Use default CRS")
                        ...
                        .clicked()
                    {
                        self.dispatch_action(AppAction::SetCrs(SetCrs::Default));
                    }
                }
                ...
                .clicked() {
                    self.dispatch_action(AppAction::OpenModal(OmapModal::SetOneCrsForAll));
                }
                if ui
                ...
                .clicked()
                {
                    self.dispatch_action(AppAction::OpenModal(OmapModal::SetOneCrsForEach));
                }
                if ui.button("Drop all non-CRS files")
                .on_hover_text("Remove all CRS-less files from the list of lidar files.")
                .clicked() {
                    self.dispatch_action(AppAction::OpenModal(OmapModal::ConfirmDropAll));
                }
            });
        });
    }
        ...
        transform_modal.show(ctx, |ui| {
            ...
            egui::Sides::new().show(
                ...
                if ui.button(format!("Majority Vote (EPSG: {})", majority_crs.epsg())).clicked() {
                    ...
                    self.dispatch_action(AppAction::OutputCrsSelected);
                }
    ...
    pub fn confirm_generate_map_modal(&mut self, ctx: &egui::Context) {
