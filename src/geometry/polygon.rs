#![allow(dead_code)]
use crate::contour::contour::Contour;
use crate::contour::vertex::Vertex;
...
pub struct Polygon{
    ...
    pub code: usize,
    pub polygon_type: PolygonTrigger,
}

impl Polygon{
    pub fn new(mut outer: Contour, polygon_type: PolygonTrigger, code: usize) -> Polygon{
        if !outer.is_closed{
            outer.close();
        }
        Polygon{
    ...
    pub fn area(&self) -> f64{
        ...
        return area*self.polygon_type as i32 as f64;
    }

    pub fn contains(&self, vertex: &Vertex) -> Result<bool, &'static str>{
        let mut inside: bool = self.boundary.contains(vertex)?;
        for hole in &self.holes{
            inside &= !hole.contains(vertex)?;
            if !inside{
                return Ok(false);
            }
        }
    ...
    pub fn polygons_from_contours(mut contours: Vec<Contour>, mut convex_hull: Contour, polygon_type: PolygonTrigger, symbol_code: usize, min_size: f64) -> Vec<Polygon>{
        ...
        while unclosed_contours.len() > 0{
            ...
            for (j, other) in unclosed_contours.iter().enumerate(){
                let dist = unclosed_contours[0].last_vertex().get_boundary_dist(&other.first_vertex(), &convex_hull).unwrap();
        ...
        match polygon_type{
            PolygonTrigger::Below => {
                ...
                polygons.push(Polygon::new(convex_hull, polygon_type, symbol_code));
            ...
            PolygonTrigger::Above => {
                for contour in closed_contours{
                    polygons.push(Polygon::new(contour, polygon_type, symbol_code));
                }
            },
        }
        ...
        while i < contours.len(){
            ...
            if area > -min_size/10. && area < min_size{
                ...
                polygons.push(Polygon::new(contour.clone(), polygon_type, symbol_code));
                contours.swap_remove(i);
            }
            else{
                i += 1;
