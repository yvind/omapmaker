...
impl Contour {
    ...
    pub fn close_by_boundary(&mut self, bounding_box: &Bounds) {
        ...
        } else if first_vertex_index == 1 {
            // xmax
            if last_vertex_index == 2 {
                // -> ymin
                let v1: Vertex = Vertex {
                    x: bounding_box.max.x,
                    y: bounding_box.min.y,
        ...
        } else if first_vertex_index == 2 {
            // ymin
            if last_vertex_index == 3 {
                // -> xmin
                let v1: Vertex = Vertex {
                    x: bounding_box.min.x,
                    y: bounding_box.min.y,
    ...
    pub fn append_by_boundary(&mut self, other: &mut Contour, bounding_box: &Bounds) {
        ...
        let dist = self
            .last_vertex()
            .get_boundary_dist(&other.first_vertex(), bounding_box)
            .unwrap();
        let edge_index1 = self.last_vertex().get_box_edge_index(bounding_box).unwrap();
        let edge_index2 = other
            .first_vertex()
            .get_box_edge_index(bounding_box)
            .unwrap();
