...
impl Matrix32x6 {
    ...
    pub fn tdot_vec(&self, vec: &Vector32) -> Vector6 {
        ...
        for i in 0..32 {
            result.data[0] += self.data[i][0] * vec.data[i];
            result.data[1] += self.data[i][1] * vec.data[i];
            result.data[2] += self.data[i][2] * vec.data[i];
            result.data[3] += self.data[i][3] * vec.data[i];
